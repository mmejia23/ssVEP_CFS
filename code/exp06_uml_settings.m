function par = exp06_uml_settings(pf_type)
...
%
% Priors are already set accordingly to results of PF for familiarity in exp05.
...
par.x0 = 0.97;    % the initial signal strength
par.x_lim = [0 1];   % the limits to the signal strength


%% Define starting point of PF estimations:

if strcmp(pf_type, 'fam')
    par.alpha = struct(...
...
elseif strcmp(pf_type, 'face')
    par.alpha = struct(...
        'limits',[0.15 0.94],...       %range of the parameter space for alpha
        'N',71,...                %number of alpha values. If this value is set to 1, then the first element of alpha.limits would be the assumed alpha and the alpha parameter is not estimated.
        'scale','lin',...         %the linear or log spacing. Choose between 'lin' and 'log'.
        'dist','norm',...         %prior distribution of the alpha parameter. Choose between 'norm' and 'flat'.
        'mu',0.55,...                %mean of the prior distribution.
        'std',0.25 ...              %standard deviation of the prior distribution.  
        );
...
par.lambda = struct(...
    ...
    'scale','lin',...         %the linear or log spacing. Choose between 'lin' and 'log'.
    'dist','norm',...         %prior distribution of the lambda parameter. Choose between 'norm' and 'flat'.
    'mu',0.02,...                %mean of the prior distribution.
    'std',0.05 ...             %standard deviation of the prior distribution.  
    );
