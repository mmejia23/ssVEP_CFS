...
% 
% Arguments:
% subject :         Vector de caracteres, cuidar que sea el mismo en ambas
%                   partes del experimento.
% 'pf', 1 :         Ejecutar primera parte, donde se estiman umbrales.
% 
% 'blockstart', 2:  Comenzar en un bloque distinto al primero.
% 
% 'debug',1 :       Correr experimento en debug, pantalla transparent.
%
% 'triggers',0 :    Correr experimento sin enviar pulsos a BrainVision.
%
% 'sim', 1 :        Correr experimento simulando respuestas del
%                   participante, ejecuta todo de corrido automaticamente.
%                   Sin esperar respuestas. Util para debug.
% 
% Info ----------------------------------------------------------------
%
...
% [data, expmnt] = ssvepCFS('NombreParticipante');
% 
% 
...
elseif pf_estimation
    ...
    if rand(1)>.5
        expmnt.main_task_block_conds_order =  [1, 2, 2, 1];
    else
        expmnt.main_task_block_conds_order =  [2, 1, 1, 2];
    end
...
expmnt.stereogram_for_pf               = [1, 2, 3, 4];
expmnt.stereogram_for_exp              = [5, 7, 8, 9, 10, 13, 15];
...
expmnt.num_masks =  ceil(n_frames_per_trial/mask_framerate)*5; % Minimum
...
if pf_estimation
    save([filename, '_desc-masks_beh.mat'], 'expmnt');
else
    save([filename, '_desc-masks_beh.mat'], 'expmnt');
end
...
% [X] Change numbering of conditions to 1 and 2. upright vs inverted faces
% [X] Set face selection to be saved and reused (or set by calling func)
