...
if any(find(strcmp(varargin, 'calculate_alpha')))
    filename = sprintf([filename_structure,'_beh.mat'], subject, 'pf');
    load(filename);
    [masks_alpha_blending1] = calculate_signal_threshold(uml_pf.fam, 0.90);
    [masks_alpha_blending2] = calculate_signal_threshold(uml_pf.fam, 0.15);
    if masks_alpha_blending1 < 0.03
        fprintf('\nEstimated alpha for visible condition was too low, correcting into = 0.03 !! \n');
        masks_alpha_blending1 = 0.03;
    end
    if masks_alpha_blending1 > 1
        fprintf('\nEstimated alpha for invisible condition was too high, correcting into = 1 !! \n\n');
        masks_alpha_blending1 = 1;
    end
    fprintf(' .......... Set alphas as: [ %.02f, %.02f ] .......\n',...
        masks_alpha_blending1, masks_alpha_blending2);
    return;
end
